...
interface AssessmentCreationResult {
 ...
 */
export async function runAssessmentCreationWorkflow(
  ...
  try {
    ...
    console.error('❌ Assessment Creation Workflow failed:', (error as Error).message);
    throw error;
  }
}
...
export async function runCleanupWorkflow(
  ...
  try {
    ...
    console.error('❌ Cleanup Workflow failed:', (error as Error).message);
    throw error;
  }
}
