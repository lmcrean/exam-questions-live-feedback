...
import { updateUserProfile, type UpdatedUser } from '../user/updateUserProfile.js';
import { deleteUser } from '../user/deleteUser.js';
import type { TestUserData } from '../auth/generateTestUser.js';
 ...
 */
export async function runUserManagementWorkflow(
  ...
  try {
    ...
    console.error('❌ User Management Workflow failed:', (error as Error).message);
    throw error;
  }
}
 ...
 */
export async function runUserDeletionWorkflow(
  request: APIRequestContext,
  expect: Expect,
  userId: string,
  authToken: string
  ...
  try {
    ...
    console.error('❌ User Deletion Workflow failed:', (error as Error).message);
    throw error;
  }
}
