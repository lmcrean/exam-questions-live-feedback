...
interface DatabaseHelloResponse {
  message: string;
  dbType: string;
  isConnected: boolean;
}
 ...
 */
export async function testDatabaseHello(
  request: APIRequestContext,
  expect: Expect
): Promise<DatabaseHelloResponse> {
  ...
  try {
    ...
    const responseData = await response.json() as DatabaseHelloResponse;
    ...
    console.error('❌ Database hello endpoint test failed:', (error as Error).message);
    throw error;
  }
}
