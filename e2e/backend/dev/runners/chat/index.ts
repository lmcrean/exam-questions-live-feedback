 ...
 */
export async function runComplete13StepWorkflow(
  ...
  try {
    ...
    const { prepareUserMessageStrings } = await import('./01-prepareUserMessageString.js');
    const { prepareAssessmentId } = await import('./02-prepareAssessmentId.js');
    const { createConversation } = await import('./03-createConversation.js');
    ...
    results.steps.step02 = { success: assessmentPrep.isValid, ...assessmentPrep };
    ...
    results.steps.step03 = { success: conversation.success, ...conversation };
