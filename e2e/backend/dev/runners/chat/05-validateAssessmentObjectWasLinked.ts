...
interface ValidationSuccessResult {
  ...
  conversation_id: string;
  assessment_id: string;
  assessment_object_accessible: boolean;
  contextual_response_received: boolean;
  response_length: number;
  assessment_object: unknown;
  validation_passed: true;
}

export type AssessmentObjectValidationResult = ValidationFailureResult | ValidationSuccessResult;
 ...
 * @returns Validation result with assessment object details
 */
export async function validateAssessmentObjectWasLinked(
  request: APIRequestContext,
  token: string,
  conversationId: string,
  assessmentId: string
): Promise<AssessmentObjectValidationResult> {
  try {
    ...
    let assessmentObject: unknown = null;
    try {
      ...
      if (assessmentResponse.status() === 200) {
        const assessmentData = await assessmentResponse.json() as AssessmentResponse;
    ...
    console.error("❌ Failed to validate assessment object linking:", (error as Error).message);
    return {
      success: false,
 ...
 * @param assessmentId - Assessment ID to check
 * @returns True if assessment object exists and is not null
 */
export async function quickCheckAssessmentObject(assessmentId: string): Promise<boolean> {
