...
interface ChatbotResponseFailureResult {
  success: false;
  error: string;
...
interface ChatbotResponseSuccessResult {
  success: true;
  response_content: string;
...
export type ChatbotResponseResult = ChatbotResponseFailureResult | ChatbotResponseSuccessResult;
 ...
 * @param expectedMessageCount - Expected total message count after response
 * @returns Validation result with response details
 */
export async function receiveChatbotResponse(
  ...
  try {
    ...
    console.error("❌ Failed to receive/validate chatbot response:", (error as Error).message);
    return {
      success: false,
