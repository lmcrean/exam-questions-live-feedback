...
export interface MessageUtilities {
  messages: string[];
 ...
 * @param index - Message index (defaults to 0)
 * @returns Test message string
 */
export function getTestUserMessage(index: number = 0): string {
    return TEST_MESSAGES[index] || TEST_MESSAGES[0];
}

/**
 * Get random test user message
 * @returns Random test message string
 */
export function getRandomTestUserMessage(): string {
    return TEST_MESSAGES[Math.floor(Math.random() * TEST_MESSAGES.length)];
}

/**
 * Get all available test messages
 * @returns Array of all test messages
 */
export function getAllTestMessages(): string[] {
    return [...TEST_MESSAGES];
}

/**
 * Get test message count
 * @returns Total number of available test messages
 */
export function getTestMessageCount(): number {
    return TEST_MESSAGES.length;
}

/**
 * Prepare user message strings with utility functions
 * @returns Message utilities object
 */
export function prepareUserMessageStrings(): MessageUtilities {
    return {
