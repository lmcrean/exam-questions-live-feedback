...
/**
 * Add assessment_object field to conversations table
 * @param db - Knex database instance
 */
export async function addAssessmentObjectToConversations(db: Knex): Promise<void> {
 ...
 * @param db - Knex database instance
 */
export async function revertAssessmentObjectFromConversations(db: Knex): Promise<void> {
  if (await db.schema.hasTable('conversations')) {
    const hasColumn = await db.schema.hasColumn('conversations', 'assessment_object');
