 ...
 */
const getScreenshotPath = (testName: string): string => `./test_screenshots/test_page/frontend-integration/auth/${testName}.png`;
 ...
 */
export async function fillRequestData(page: Page, data: Record<string, unknown>): Promise<void> {
 ...
 * @param page Playwright page
 * @returns Test state with user data
 */
export async function runAuthTests(page: Page): Promise<AuthTestState> {
  const testState: AuthTestState = {
 ...
 */
export async function testRegister(page: Page, testState: AuthTestState): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // Ignore errors when looking for response
    }
    ...
    if (responseText) {
      ...
      try {
        ...
        if (match) {
          responseData = JSON.parse(match[0]) as RegisterResponse;
 ...
 * @param testState Test state with user data
 */
export async function testLogin(page: Page, testState: AuthTestState): Promise<void> {
  ...
  // Verify response content
  const responseData = await getResponseData(page) as LoginResponse | null;
