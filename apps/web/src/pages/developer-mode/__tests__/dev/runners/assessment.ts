 ...
 */
const getScreenshotPath = (testName: string): string => `./test_screenshots/test_page/frontend-integration/assessment/${testName}.png`;

/**
 * Runs all assessment endpoint tests
 * @param page Playwright page
 * @param testState Test state with user data
 * @returns Updated test state with assessment IDs
 */
export async function runAssessmentTests(page: Page, testState: AssessmentTestState): Promise<AssessmentTestState> {
 ...
 * @param testState Test state with user data
 */
export async function testCreateAssessment(page: Page, testState: AssessmentTestState): Promise<void> {
  ...
  const responseData = await getResponseData(page) as AssessmentResponse | null;
 ...
 * @param testState Test state with user data
 */
export async function testGetAssessments(page: Page, testState: AssessmentTestState): Promise<void> {
  ...
  // Verify response content
  const assessmentsList = await getResponseData(page) as Assessment[] | null;
  ...
  const hasAssessment = assessmentsList?.some((a: Assessment) => a.id === testState.assessmentIds[0]);
 ...
 * @param testState Test state with user data
 */
export async function testCreateSecondAssessment(page: Page, testState: AssessmentTestState): Promise<void> {
 ...
 * @param testState Test state with user data
 */
export async function testGetAssessmentById(page: Page, testState: AssessmentTestState): Promise<void> {
  ...
  // Verify response content
  const assessment = await getResponseData(page) as AssessmentResponse | null;
 ...
 * @param testState Test state with user data
 */
export async function testDeleteAssessments(page: Page, testState: AssessmentTestState): Promise<void> {
  ...
  for (const assessmentId of testState.assessmentIds) {
    ...
    // Verify response content
    const deleteResponse = await getResponseData(page) as DeleteResponse | null;
  ...
  const remainingAssessments = await getResponseData(page) as Assessment[] | null;

  // Check no test assessments remain
  for (const assessmentId of testState.assessmentIds) {
    const stillExists = remainingAssessments?.some((a: Assessment) => a.id === assessmentId);
