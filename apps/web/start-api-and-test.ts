...
function isPortInUse(port: number | string): boolean {
  try {
...
if (isPortInUse(API_PORT)) {
  ...
  const serverProcess: ChildProcess = spawn('node', [path.join(__dirname, '..', 'backend', 'server.js')], {
    stdio: 'inherit',
...
function runTests(): void {
  ...
  const testProcess: ChildProcess = spawn('npm', ['test', '--', 'GetApiMessage'], {
    stdio: 'inherit',
    shell: true
  });
