...
export async function signUpTestUser(page: Page): Promise<UserCredentials> {
  ...
  console.log(
    signUpButton
      ? 'Found button with selector: button:has-text("Create account")'
      : 'Could not find sign-up button'
  );
  ...
  try {
    ...
    // Note: successElement check available but not used in current logic
    ...
    if (errorElement > 0) {
      const errorMessage = await page
        .locator('.error-message, [data-testid="signup-error"]')
        .textContent();
