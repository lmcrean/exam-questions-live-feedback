 ...
 */
export async function addAssessmentFieldsToConversations(db: Knex): Promise<void> {
    ...
    await db.schema.table('conversations', (table) => {
      ...
      if (!isSQLite && !hasAssessmentId) {
        try {
          ...
          console.warn('Warning: Could not create foreign key for assessment_id:', (error as Error).message);
        }
      }
    });
 ...
 * @param db - Knex database instance
 */
export async function revertAssessmentFieldsFromConversations(db: Knex): Promise<void> {
  if (await db.schema.hasTable('conversations')) {
    await db.schema.table('conversations', (table) => {
