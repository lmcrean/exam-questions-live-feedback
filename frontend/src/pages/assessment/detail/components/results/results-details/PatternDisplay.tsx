  ...
  if (variant === 'compact') {
    return (
      <div className="flex items-center gap-3 rounded-lg border bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <img src={patternInfo.icon} className="h-12 w-12" alt={`${patternInfo.title} icon`} />
        <div>
          <h3 className="font-semibold text-pink-600 dark:text-pink-400">{patternInfo.title}</h3>
          {showDescription && (
            <p className="text-sm text-gray-600 dark:text-slate-300">{patternInfo.description}</p>
          )}
        </div>
      </div>
    );
