...
/**
 * Update assessment schema to flattened structure
 * @param db - Knex database instance
 */
export async function updateFlattenedAssessmentSchema(db: Knex): Promise<void> {
  ...
  if (hasTable) {
    ...
    await db.schema.alterTable("assessments", async (table) => {
      // Add the flattened fields from assessment_data
      if (!(await hasColumn(db, "assessments", "age"))) table.string("age");
      if (!(await hasColumn(db, "assessments", "pattern"))) table.string("pattern");
      if (!(await hasColumn(db, "assessments", "cycle_length"))) table.string("cycle_length");
      if (!(await hasColumn(db, "assessments", "period_duration"))) table.string("period_duration");
      if (!(await hasColumn(db, "assessments", "flow_heaviness"))) table.string("flow_heaviness");
      if (!(await hasColumn(db, "assessments", "pain_level"))) table.string("pain_level");

      // Add columns for arrays with JSON format
      if (!(await hasColumn(db, "assessments", "physical_symptoms"))) table.text("physical_symptoms");
      if (!(await hasColumn(db, "assessments", "emotional_symptoms"))) table.text("emotional_symptoms");
      if (!(await hasColumn(db, "assessments", "recommendations"))) table.text("recommendations");
    });
 ...
 * @param db - Knex database instance
 */
export async function revertFlattenedAssessmentSchema(db: Knex): Promise<void> {
  // Check if assessments table exists
